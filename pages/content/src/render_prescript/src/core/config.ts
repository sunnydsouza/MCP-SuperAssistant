...
export const WEBSITE_CONFIGS: Array<{
  ...
  {
    // Microsoft Copilot uses a richer code-block editor for some languages (including
    // jsonl, which it may render as Plain Text) instead of always emitting a native
    // <pre>/<code> tree. Include Monaco-style line containers in addition to the
    // normal targets so the JSONL execution-request emitted by Copilot is observed.
    urlPattern: /(?:copilot|m365)\.cloud\.microsoft/,
    config: {
      targetSelectors: ['pre', 'code', '.monaco-editor .view-lines', '.view-lines', '[role="code"]'],
      streamingContainerSelectors: ['pre', 'code', '.monaco-editor', '.view-lines', '[role="code"]'],
  ...
  },
  {
    ...
    config: {
      targetSelectors: ['pre'],
      streamingContainerSelectors: ['pre'],
      function_result_selector: ['div[data-testid="user-message"]'],
    },
  },
  {
